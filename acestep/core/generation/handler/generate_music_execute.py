...
def _parse_generation_timeout() -> int:
    ...
    if val <= 0:
        logger.warning(
            "ACESTEP_GENERATION_TIMEOUT=%d must be positive; defaulting to 600s.", val
        )
        return 600
    return val
...
class GenerateMusicExecuteMixin:
    ...
    def _run_generate_music_service_with_progress(
        ...
        Raises:
            ...
            Exception: any exception raised by ``service_generate`` is re-raised
        ...
        try:
            ...
            if gen_thread.is_alive():
                ...
                logger.error(
                    "[generate_music] service_generate exceeded %ds timeout "
                    "(batch=%d, steps=%d, duration=%.1fs). "
